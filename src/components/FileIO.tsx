...
export default function FileIO() {
	...
	const download = useCallback(() => {
		...
		const file = new Blob(
			[ JSON.stringify(storedStore, undefined, '\t') ],
	...
	return <div className="flex gap-2">
		<button onClick={download} className={className}>Download</button>
		<button onClick={upload} className={className}>Upload</button>
	</div>;
}
